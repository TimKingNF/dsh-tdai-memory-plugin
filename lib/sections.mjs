...
function registerKnowledgeSkill(ctx, runtime, log, registered) {
  ...
  return (session, asset) => {
    ...
    // 子 agent 会话按策略降级：不注册知识 skill（它的任务是父 agent 指定的具体工作，
    // 一行路由也是噪音）。见 lib/subagent.mjs。
    if (!readSideAllowed(runtime.config, session)) return
...
export function registerSections(ctx, runtime) {
  ...
  if (typeof ctx.systemPrompt.context === 'function') {
    ctx.systemPrompt.context({
      ...
      text: (context) => {
        ...
        const sessionId = session?.id
        if (!sessionId) return ''
        // 子 agent 会话按策略降级：状态快照对范围明确的子任务是纯噪音（lib/subagent.mjs）。
        if (!readSideAllowed(runtime.config, session)) return ''
        // 只读缓存：这一步在热路径上，绝不发请求。
  ...
  ctx.on('system-prompt/assemble', async (assembly, context, next) => {
    ...
    // 子 agent 会话按策略降级（默认）：**直接不改写 section**。
    //
    // 四个 section 注册时的同步 text 本来就是空占位（内容只在这里覆盖），所以
    // "不改写"= 段文本保持为空 = DSH 的 renderPrompt 会把空段整个丢掉 → 子会话的
    // system prompt 里连标签都不会出现。顺带这里也跳过了 `assets.ensure()`：
    // 子会话不再为一份用不到的资产打一轮 HTTP（并发扇出时这是 N 份）。
    if (!readSideAllowed(runtime.config, session)) return result
    ...
    ensureKnowledgeSkill(session, asset)
